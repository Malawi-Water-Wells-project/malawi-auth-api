"""
Created 01/03/2021
API Resources for /tribes
"""

import base64
import json

from app.main.constants import UserRoles
from app.main.dto import TribeDto
from app.main.service.tribe_service import (TribeSearchQuery, check_join_token,
                                            create_tribe_join_token,
                                            get_tribe_by_id,
                                            get_tribe_by_public_id,
                                            lookup_join_token, save_new_tribe)
from app.main.service.user_service import (create_new_user,
                                           find_user_by_username,
                                           get_admins_by_tribe)
from app.main.util.decorator import user_is_tribe_admin, user_logged_in
from app.main.util.jwt import generate_jwt_keypair
from flask.globals import request
from flask_restx import Resource, reqparse

api = TribeDto.api
_tribe = TribeDto.tribe
_admin = TribeDto.admin
_new_user = TribeDto.new_user


@api.route("/")
class GetTribe(Resource):
    """ API Resource for /tribes """

    @api.doc("Gets the current tribe")
    @user_logged_in
    def get(self, jwt):
        """
        GET /tribes
        Gets the Tribe for the currently logged in user
        """
        tribe_id = jwt.get("tribe_id")
        tribe = get_tribe_by_id(tribe_id)

        return {
            "status": "Success",
            "tribe": tribe.dictionary
        }


@api.route("/<string:tribe_id>")
class GetTribeByID(Resource):
    """ API Resource for /tribes/<tribe_id> """

    @api.doc("Retrives a Tribe by its public ID")
    def get(self, tribe_id):
        """
        GET /tribes/<tribe_id>
        Gets a tribe by public ID
        """
        tribe = get_tribe_by_public_id(tribe_id)

        if tribe is None:
            return {
                "status": "Failure"
            }, 404

        return {
            "status": "Success",
            "tribe": tribe.dictionary
        }


@api.route("/<string:tribe_id>/admins")
class GetAdminsForTribe(Resource):
    """ Resource for /tribes/<tribe_id>/admins """

    @api.doc("Gets all Tribe Admins for a tribe")
    def get(self, tribe_id):
        """
        GET /tribes/<tribe_id>/admins
        Gets all Tribe Admins for a Tribe
        """
        tribe = get_tribe_by_public_id(tribe_id)

        if not tribe:
            return {
                "status": "Failure",
                "message": "Tribe not found"
            }, 404

        admins = get_admins_by_tribe(tribe.id)

        return {
            "status": "Success",
            "admins": [admin.dictionary for admin in admins]
        }


@api.route("/create")
class CreateTribe(Resource):
    """ Resource for /tribes/create """

    @api.doc("Create a new Tribe")
    @api.response(201, "Tribe created successfully")
    @api.expect(_tribe, validate=True)
    def post(self):
        """
        POST /tribes/create
        Creates a new Tribe
        """
        new_tribe = save_new_tribe(request.json)
        return {
            "status": "Success",
            "tribe": new_tribe.dictionary
        }, 201


@api.route("/<tribe_id>/admin")
class TribeAdmin(Resource):
    """ Resource for /tribes/<tribe_id>/admin """

    @api.doc("Create a new tribe admin")
    @api.response(201, "Tribe Admin added successfully")
    @api.response(404, "Tribe not found")
    @api.expect(_admin, validate=True)
    def post(self, tribe_id):
        """
        POST /tribes/<tribe_id>/admin
        Creates a new Tribe Admin
        """
        tribe = get_tribe_by_public_id(tribe_id)
        if tribe is None:
            return {
                "status": "Failure",
                "message": "Tribe not found"
            }, 404

        existing_user = find_user_by_username(request.json.get("username"))
        if existing_user is not None:
            return {
                "status": "Failure",
                "message": "Username is already taken"
            }, 400

        new_user = create_new_user(
            request.json,
            tribe.id,
            UserRoles.TRIBE_ADMIN
        )

        return {
            "status": "Success",
            "user": new_user.dictionary
        }, 201


@api.route("/check-token")
class CheckToken(Resource):
    """ Resource for /tribes/check-token """

    def post(self):
        """
        POST /tribes/check-token
        Checks if a join token is valid, and returns Tribe details if it is
        """
        token = request.json["token"]

        if token is None:
            return {}, 500

        decoded_token = json.loads(base64.b64decode(
            token.encode("ascii")).decode("ascii"))

        is_valid = check_join_token(decoded_token)

        if not is_valid:
            return {}, 500

        return {
            "status": "Success",
            "isValid": True,
            "token": decoded_token
        }


@api.route("/join")
class JoinTribe(Resource):
    """ Resource for /tribes/join """
    @api.doc("Creates a standard user associated with a tribe")
    @api.expect(_new_user, validate=True)
    def post(self):
        """
        POST /tribes/join
        Creates a standard user associated with the token provided
        """

        token = lookup_join_token(request.json["token"])

        if token is None:
            return {}, 400

        tribe = get_tribe_by_public_id(token.get("tribe_id"))
        if tribe is None:
            return {}, 400

        user = create_new_user(request.json, tribe.id, UserRoles.USER)

        access_token, refresh_token = generate_jwt_keypair(
            user.id, tribe.id, user.role)

        return {
            "status": "Success",
            "user": user.dictionary,
            "tokens": {
                "access": access_token,
                "refresh": refresh_token
            }
        }


@api.route("/<tribe_id>/token")
class TribeToken(Resource):
    """ Resource for /tribes/<tribe_id>/token """

    @api.doc("Creates a Join Token for a Tribe")
    @user_logged_in
    @user_is_tribe_admin
    def get(self, jwt, tribe_id, tribe):
        """
        GET /tribes/<tribe_id>/token
        Creates a Join Token that can be standard users can use to join a tribe
        """

        token = create_tribe_join_token(tribe.public_id, tribe.name)

        return {
            "status": "Success",
            "token": str(base64.encodebytes(json.dumps(token).encode()), "utf-8")
        }


@api.route("/search")
class TribeSearch(Resource):
    """ Resource for /tribes/search """

    @api.doc("Searches for Tribes")
    def get(self):
        """
        GET /tribes/search
        Query for Tribes
        """
        search_query = self._parse_request()

        results = search_query.run_query()

        return {
            "status": "Success",
            "count": len(results),
            "results": [result.dictionary for result in results]
        }

    def _parse_request(self) -> TribeSearchQuery:
        """ Parser for the incoming request """
        parser = reqparse.RequestParser()
        parser.add_argument("type", type=str, required=True,
                            help="Type is required", location="args")
        parser.add_argument("query", type=str, location="args")
        parser.add_argument("latitude", type=float, location="args")
        parser.add_argument("longitude", type=float, location="args")
        parser.add_argument("radius", type=int, location="args")

        args = parser.parse_args()

        search_type = args.get("type", None)

        if search_type not in ["name", "location"]:
            return

        if search_type == "location":
            return self._parse_location(args)

    def _parse_location(self, args) -> TribeSearchQuery:
        """ Parse the lat/long and radius from the arguments """
        latitude = args.get("latitude", None)
        longitude = args.get("longitude", None)
        radius = args.get("radius", None)

        if not latitude or not longitude:
            return {}

        return TribeSearchQuery(
            TribeSearchQuery.LOCATION_TYPE,
            latitude=latitude,
            longitude=longitude,
            radius=radius
        )
